module Applications
  module Bi
    class PopulateSummonersPerformance
      def execute
        update_klass = Applications::Bi::Summoners::CreateSummonerPerformance.new
        ...
        summoners.each do |summoner|
          ...
          # Log
          puts "Updating SummonerPerformance: #{summoner.summoner_name}"
      ...
      def move_to_next?(summoner_id, summoner_matches)
        ...
        previous_total_games_played = (SummonerPerformance.where(summoner_id: summoner_id).order(total_games: :desc).limit(1).pluck(:total_games).first rescue nil)
